...
export default function TenantLeaseViewPage() {
  ...
  const fetchLeaseData = async () => {
    try {
      // First get the user's lease to get the lease ID
      ...
      // Then fetch the full lease agreement with signature data
      const agreementRes = await fetch(`/api/lease-agreement/${lease.id}`)
      if (!agreementRes.ok) throw new Error('Failed to fetch lease agreement')
      const agreement = await agreementRes.json()
      setLeaseData({ ...agreement.lease, signature: agreement.signature })
    } catch (err: any) {
      ...
      <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-slate-50 to-slate-100">
        <Card className="w-full max-w-3xl">
          <CardContent className="p-8 text-center">
            <p className="text-gray-600">Loading your lease agreement...</p>
          </CardContent>
        </Card>
      </div>
    )
  ...
  if (error || !leaseData) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-slate-50 to-slate-100">
        <Card className="w-full max-w-3xl">
          <CardContent className="p-8 text-center">
            <p className="text-red-600 mb-4">{error || 'No lease found'}</p>
    ...
    <div className="min-h-screen py-8 px-4 bg-gradient-to-br from-slate-50 to-slate-100">
      <div className="max-w-3xl mx-auto">
        <div className="mb-6 flex justify-between items-center">
          <h1 className="text-3xl font-bold">Your Lease Agreement</h1>
              ...
              <div className="mt-10 pt-6 border-t-2 border-gray-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-center mt-2 text-sm font-semibold text-gray-800">{tenantFullName}</p>
                    {leaseData.signed_at && (
